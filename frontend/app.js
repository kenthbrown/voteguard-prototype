...
function updateVotingState() {
  if (sessionExpired) {
    voteStatusPill.textContent = "Session Expired";
    sessionState.textContent = "This session expired after the short demo timeout.";
...
function startSessionTimeout(durationMs) {
  clearSessionTimeout();
  ...
  sessionTimeoutHandle = setTimeout(async () => {
    ...
    voteConfirmation.classList.add("hidden");
    voteConfirmation.textContent = "";
    statusMessage.textContent = "Session expired. Please start a new session.";
    updateVotingState();

    await fetch("/session-expired", {
      method: "POST",
      headers: {
...
loginForm.addEventListener("submit", async (event) => {
  ...
  voteConfirmation.textContent = "";
  updateVotingState();
  startSessionTimeout(data.expiresInMs || 2 * 60 * 1000);
  await loadAuditLog();
});
